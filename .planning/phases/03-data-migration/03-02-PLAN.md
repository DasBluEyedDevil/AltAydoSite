...
<task type="auto">
  <name>Task 1: Create migration script scaffolding with users, missions, and planned-missions handlers</name>
...
**Important:** Leave `migrateOperations` and `migrateResources` as stub functions that log `"MIGRATION: [operations|resources] -- handler pending (Task 2)"` and return immediately. Also leave `printReport` as a stub that logs `"MIGRATION: Report printing pending (Task 2)"`. These will be implemented in Task 2.
  </action>
  <verify>
...
- Verify main() calls all 5 migration functions:
  - `grep "migrateUserShips" src/scripts/migrate-ship-references.ts` -- found
  - `grep "migrateMissions" src/scripts/migrate-ship-references.ts` -- found
  - `grep "migratePlannedMissions" src/scripts/migrate-ship-references.ts` -- found
  - `grep "migrateOperations" src/scripts/migrate-ship-references.ts` -- found
  - `grep "migrateResources" src/scripts/migrate-ship-references.ts` -- found
- Verify the script does NOT contain "deleteField", "unset", "$unset", or "delete ship.name" (must not remove existing fields)
  </verify>
  <done>
Migration script exists at `src/scripts/migrate-ship-references.ts` with complete implementations for `migrateUserShips`, `migrateMissions`, `migratePlannedMissions`, and stubs for `migrateOperations`, `migrateResources`, `printReport`. The `main()` function calls all 5 handlers sequentially. Supports `--dry-run` flag. Type-check passes. Does not remove any existing fields.
  </done>
</task>

<task type="auto">
  <name>Task 2: Implement operations, resources handlers and printReport</name>
  <files>src/scripts/migrate-ship-references.ts</files>
  <action>
Replace the stub implementations of `migrateOperations`, `migrateResources`, and `printReport` in `src/scripts/migrate-ship-references.ts` with full implementations.
...
**Ensure `fs` and `path` are imported** at the top of the file (add if not already present from Task 1).

**Critical constraints:**
- Do NOT remove any existing fields -- ONLY ADD `fleetyardsId`
- Do NOT use bulk operations -- update one document at a time with try/catch
- Use direct fs for operations.json and resources.json
  </action>
  <verify>
Run `npm run type-check` -- zero errors.
Verify all stubs are now real implementations:
...
- `grep "printReport" src/scripts/migrate-ship-references.ts` -- found
- `grep "SHIP REFERENCE MIGRATION REPORT" src/scripts/migrate-ship-references.ts` -- report header present
- `grep "handler pending" src/scripts/migrate-ship-references.ts` -- NOT found (stubs replaced)
- Verify main() still calls all 5 migration functions:
  - `grep "migrateUserShips" src/scripts/migrate-ship-references.ts` -- found
  - `grep "migrateMissions" src/scripts/migrate-ship-references.ts` -- found
  - `grep "migratePlannedMissions" src/scripts/migrate-ship-references.ts` -- found
  - `grep "migrateOperations" src/scripts/migrate-ship-references.ts` -- found
  ...
  <done>
All 5 collection handlers are fully implemented (no stubs remain). `printReport` outputs a comprehensive structured report. Type-check passes. The script handles operations.json and resources.json via direct fs operations.
  </done>
</task>

<task type="auto">
  <name>Task 3: Add npm script and verify type-check</name>
...
- `main()` calls all 5 functions: `migrateUserShips`, `migrateMissions`, `migratePlannedMissions`, `migrateOperations`, `migrateResources`
</verification>
